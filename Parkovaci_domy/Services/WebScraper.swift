...
class WebScraper {
    ...
    private let parkHouseUrl : String = "https://www.parkovanivbrne.cz/parkovani-dalsi"
    
    public func getParkHouses() -> [ParkHouse] {
        
        var houseArray : [ParkHouse] = []
        var htmlData : String
        var htmlDoc : Document
        
        if let url = URL(string: self.parkHouseUrl) {
            do {
        ...
        do {
            ...
            for house in houses {
                ...
                let HouseStruct : ParkHouse? = self.convertToStruct(from: house)
    ...
    private func convertToStruct(from house : Element) -> ParkHouse? {
